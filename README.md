# Lineare Algebra und Geometrie (LAG)

This repository contains learning materials, notebooks, and exercises for the **Linear Algebra and Geometry** course.  
The focus is on **linear equation systems, matrices, Gaussian elimination, and Python tools (NumPy / SymPy)**.

---

# Chapter 1 – Linear Equation Systems

Video series covering the foundations of **linear equations, matrices, and solving systems using the Gauss algorithm**.

| # | Status | Topic | Description | Duration | Video |
|---|---|---|---|---|---|
| 1 | ✅ | Motivation / Introduction | Introduction to linear systems through a motivating example (sorting comparisons vs list size). | 12:00 | https://tube.switch.ch/videos/MAF6mSw9I7 |
| 2 | ✅ | Linear Equation | What is a linear equation with *n variables*. Visualization of solution sets (lines in 2D). | 11:31 | https://tube.switch.ch/videos/JfVaMA9hz3 |
| 3 | ✅ | Linear Equation Systems | Definition of linear systems. Solutions as intersections of solution sets (lines and planes). | 08:33 | https://tube.switch.ch/videos/kIj7nmhu2F |
| 4 | ✅ | Matrix Representation | Matrix representation of linear systems. Coefficient matrix and augmented matrix. | 09:17 | https://tube.switch.ch/videos/3uc8j0mzbh |
| 5 | ✅ | Classical Solution Methods | Review of substitution, elimination, and comparison methods. | 10:01 | https://tube.switch.ch/videos/YliyiQMPmD |
| 6 | ✅ | Row Transformations | Equivalent transformations and elementary row operations. | 19:30 | https://tube.switch.ch/videos/4YmuqDI0ty |
| 7 | ✅ | Gauss & Gauss-Jordan | Gaussian elimination and Gauss-Jordan algorithm. Introduction of row echelon form and reduced row echelon form. | 19:27 | https://tube.switch.ch/videos/A6m3ohxIwn |
| 8 | ✅ | Solution Cases / Matrix Rank | Unique solution, infinitely many solutions, or no solution. Use of matrix rank to determine cases. | 23:08 | https://tube.switch.ch/videos/C80pjtCzhw |
| 9 | ✅ | Example Exercises | Worked examples applying the methods of Chapter 1. | 14:28 | https://tube.switch.ch/videos/0cCLmYBoa6 |
| 10 | ✅ | Python | Important Python commands for working with linear systems. | 19:29 | https://tube.switch.ch/videos/2qRMSONrWV |

---

# Chapter 2 – Matrix Algebra

Video series covering **matrix operations, determinants, inverses, and practical matrix computations**.

| # | Status | Topic | Description | Duration | Video |
|---|---|---|---|---|---|
| 1 | ✅ | Introduction / Motivation | Introductory example (Markov chain) showing that matrices are more than just data storage. | 11:44 | https://tube.switch.ch/videos/PLyIMuJoo5 |
| 2 | ✅ | Matrices / Terminology / Special Matrices | Important matrix concepts: square matrices, symmetric matrices, zero matrix, identity matrix, etc. | 09:44 | https://tube.switch.ch/videos/tHWBzwja6P |
| 3 | ✅ | Matrix Operations 1 | Matrix addition, subtraction, and scalar multiplication with motivating examples. | 08:44 | https://tube.switch.ch/videos/KUrLBIC0i5 |
| 4 | ✅ | Matrix Operations 2 | Matrix multiplication explained step by step with examples and applications. | 18:00 | https://tube.switch.ch/videos/sglUlnAw3h |
| 5 | ✅ | Transpose of a Matrix | Definition of matrix transposition and its laws. Example application in linear regression. | 12:53 | https://tube.switch.ch/videos/mBbN7B9Cze |
| 6 | ✅ | Inverse of a Matrix | Definition of matrix inverse, Gauss-Jordan method, invertibility conditions, and properties. | 20:03 | https://tube.switch.ch/videos/8cDtaKze8w |
| 7 | ⬜ | Determinant 1 / Cramer's Rule | Determinants of 2×2 and 3×3 matrices and solving systems using Cramer's rule. | 15:26 | https://tube.switch.ch/videos/Y88oNylvJE |
| 8 | ⬜ | Determinant 2 – General Definition | General determinant definition using permutations and factorial concepts. | 11:47 | https://tube.switch.ch/videos/Hi0MKP7Jmt |
| 9 | ⬜ | Determinant 3 – Gaussian Method | Determinant calculation using the Gaussian elimination method. | 14:41 | https://tube.switch.ch/videos/sNw5BWrUuA |
| 10 | ⬜ | Determinant 4 – Laplace Expansion | Determinant computation using the recursive Laplace expansion theorem. | 11:52 | https://tube.switch.ch/videos/JiMYJotwQH |
| 11 | ⬜ | Adjugate Matrix / Inverse Formula | Introduction of the adjugate matrix and formula for computing matrix inverses. | 11:50 | https://tube.switch.ch/videos/GcBRrq5b3X |
| 12 | ⬜ | Example Exercises | Worked exercises demonstrating the tools and concepts of Chapter 2. | 24:31 | https://tube.switch.ch/videos/oVymOUitwY |
| 13 | ⬜ | Matrix Algebra with Python | Matrix operations (NumPy / SymPy) and implementing determinant algorithms (Gauss / Laplace). | 01:50 | https://tube.switch.ch/videos/wV0bxNeLGw |

---

# Python Tools Used

This repository includes examples using:

- **NumPy** – numerical linear algebra
- **SymPy** – symbolic mathematics
- **Jupyter Notebook** – interactive computation

These notebooks are designed to help you practice linear algebra concepts interactively using Python.

## Jupyter Notebooks (Run in Google Colab)

You can open and run all notebooks directly in your browser using Google Colab (no installation required):

- Einführung NumPy  
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/inspiringsource/Lineare_Algebra_und_Geometrie-lag-/blob/main/Einf%C3%BChrung%20NumPy.ipynb)

- Einführung SymPy  
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/inspiringsource/Lineare_Algebra_und_Geometrie-lag-/blob/main/Einf%C3%BChrung%20SymPy.ipynb)

- Erste Schritte mit Python und Jupyter  
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/inspiringsource/Lineare_Algebra_und_Geometrie-lag-/blob/main/ErsteSchritteMitPythonUndJupyter.ipynb)

- Übung 2  
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/inspiringsource/Lineare_Algebra_und_Geometrie-lag-/blob/main/uebung-2-python.ipynb)

- Übung 3 (Solution)  
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/inspiringsource/Lineare_Algebra_und_Geometrie-lag-/blob/main/uebung-3-sol-python.ipynb)

- Übung 4 (Solution)  
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/inspiringsource/Lineare_Algebra_und_Geometrie-lag-/blob/main/uebung-4-sol-python.ipynb)

- Übung 5 (Solution)  
  [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/inspiringsource/Lineare_Algebra_und_Geometrie-lag-/blob/main/uebung-5-sol-python.ipynb)

---

# Goal of this Repository

The goal is to understand Lineare Algebra und Geometrie (lag) by documenting progress through video lectures, exercises, and Python implementations.

---

# Author

Avi B